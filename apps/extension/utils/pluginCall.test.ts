...
describe("normalizePluginCall", () => {
  describe("拒绝非法 payload", () => {
    it("payload 为 null / undefined / 字符串 → 拒绝", () => {
      ...
      expect(normalizePluginCall("hi" as any).ok).toBe(false);
      expect(normalizePluginCall(123 as any).ok).toBe(false);
    });
    ...
    it("value 是空字符串或纯空白 → 拒绝", () => {
      expect(normalizePluginCall({ type: "sendMessage", value: "" }).ok).toBe(
        false
      );
      expect(
        normalizePluginCall({ type: "sendMessage", value: "   \n  " }).ok
      ).toBe(false);
    });
  });
